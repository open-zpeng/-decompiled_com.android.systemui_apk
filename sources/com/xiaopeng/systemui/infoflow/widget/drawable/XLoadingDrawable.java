...
public class XLoadingDrawable extends Drawable {
    ...
    private static final float DELAY_FACTOR_LARGE = 0.15f;
    private static final float DELAY_FACTOR_MEDIUM = 0.15f;
    private static final float DELAY_FACTOR_SMALL = 0.2f;
    private static final float DELAY_FACTOR_XLARGE = 0.1f;
    ...
    private static final String TAG = "XLoadingDrawable";
    ...
    private void makeAnimations() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            this.mAnimations[index].addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.xiaopeng.systemui.infoflow.widget.drawable.XLoadingDrawable.1
                @Override // android.animation.ValueAnimator.AnimatorUpdateListener
                public void onAnimationUpdate(ValueAnimator animation) {
                    float value = ((Float) animation.getAnimatedValue()).floatValue();
                    XLoadingDrawable.this.mAlphas[index] = ((int) (163.0f * value)) + 92;
                    XLoadingDrawable.this.mRectHeights[index] = XLoadingDrawable.this.minHeight + ((XLoadingDrawable.this.maxHeight - XLoadingDrawable.this.minHeight) * value);
                    float[] fArr = XLoadingDrawable.this.mRectTopYs;
                    int i2 = index;
                    float f = XLoadingDrawable.this.mCenterY;
                    float[] fArr2 = XLoadingDrawable.this.mRectHeights;
                    int i3 = index;
                    fArr[i2] = (f - (fArr2[i3] * 0.5f)) + ((((count - 1) * 0.5f) - i3) * ((float) Math.tan(XLoadingDrawable.DEFAULT_RADIANS)) * (XLoadingDrawable.this.mSpacingX + XLoadingDrawable.this.mRectWidth));
                    if (index == count - 1) {
                        XLoadingDrawable.this.invalidateSelf();
                    }
                }
            });
        }
    ...
    public void onConfigurationChanged(Context context, Configuration newConfig) {
        if (ThemeManager.isThemeChanged(newConfig)) {
            this.mColor = context.getResources().getColor(R.color.x_theme_text_01);
            setBlurEffect();
            invalidateSelf();
        }
    }
    ...
    private void setBlurEffect() {
        ...
        if (ThemeManager.isNightMode(SystemUIApplication.getContext())) {
    ...
    private void initParams() {
        if (this.mCount == 0) {
            Log.e(TAG, "You must setType or config loading_type first");
        }
        if (this.mCenterX == 0.0f) {
            this.mCenterX = getIntrinsicWidth() * 0.5f;
            this.mCenterY = getIntrinsicHeight() * 0.5f;
            this.mColor = SystemUIApplication.getContext().getResources().getColor(R.color.x_theme_text_01);
            int i = this.mCount;
