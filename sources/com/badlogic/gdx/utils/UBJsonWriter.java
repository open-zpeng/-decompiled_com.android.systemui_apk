...
public class UBJsonWriter implements Closeable {
    ...
    public UBJsonWriter name(String name) throws IOException {
        ...
        byte[] bytes = name.getBytes(Key.STRING_CHARSET_NAME);
        if (bytes.length <= 127) {
    ...
    public UBJsonWriter value(String value) throws IOException {
        checkName();
        byte[] bytes = value.getBytes(Key.STRING_CHARSET_NAME);
    ...
    public class JsonObject {
        ...
        void close() throws IOException {
            UBJsonWriter.this.out.writeByte(this.array ? 93 : SpeechConstant.SoundLocation.DEF_PASSENGER_ANGLE);
        }
    }
}
