    ...
    public void addPatchFile(String zipFileName) throws IOException {
        ...
        if (header == kEOCDSignature) {
            ...
            if (dirOffset + dirSize > fileLength) {
                Log.w(LOG_TAG, "bad offsets (dir " + dirOffset + ", size " + dirSize + ", eocd " + eocdIdx + NavigationBarInflaterView.KEY_CODE_END);
                throw new IOException();
                ...
                while (i < numEntries) {
                    ...
                    if (directoryMap.getInt(currentOffset) != kCDESignature) {
                        Log.w(LOG_TAG, "Missed a central dir sig (at " + currentOffset + NavigationBarInflaterView.KEY_CODE_END);
                        throw new IOException();
                    }
