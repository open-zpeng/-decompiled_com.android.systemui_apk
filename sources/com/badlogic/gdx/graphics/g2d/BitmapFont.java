...
public class BitmapFont implements Disposable {
    ...
    public void setFixedWidthGlyphs(CharSequence glyphs) {
        ...
        for (int index2 = 0; index2 < end2; index2++) {
            ...
            if (g2 != null) {
                g2.xoffset += Math.round((maxAdvance - g2.xadvance) / 2);
    ...
    public static class BitmapFontData {
        ...
        public void getGlyphs(GlyphLayout.GlyphRun run, CharSequence str, int start, int end, Glyph lastGlyph) {
            ...
            while (start < end) {
                ...
                if (ch != '\r') {
                    ...
                    lastGlyph = glyph;
                    start = (markupEnabled && ch == '[' && start2 < end && str.charAt(start2) == '[') ? start2 + 1 : start2;
                }
                start = start2;
            }
            if (lastGlyph != null) {
        ...
        public int getWrapIndex(Array<Glyph> glyphs, int start) {
            ...
            while (i > 0) {
                char ch2 = (char) glyphs.get(i).id;
                if (!isBreakChar(ch2) && !isWhitespace(ch2)) {
                    i--;
                }
                return i + 1;
            }
