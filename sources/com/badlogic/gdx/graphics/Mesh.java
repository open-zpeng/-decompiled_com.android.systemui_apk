        ...
        if (this.isVertexArray) {
            ...
            if (this.indices.getNumIndices() > 0) {
                if (count + offset > this.indices.getNumMaxIndices()) {
                    throw new GdxRuntimeException("Mesh attempting to access memory outside of the index buffer (count: " + count + ", offset: " + offset + ", max: " + this.indices.getNumMaxIndices() + NavigationBarInflaterView.KEY_CODE_END);
        ...
        if (i != 1) {
            if (i != 2) {
                ...
                for (int i4 = offset; i4 < end; i4++) {
                    int idx3 = (index.get(i4) * vertexSize) + posoff;
            ...
            for (int i6 = offset; i6 < end; i6++) {
                int idx5 = (index.get(i6) * vertexSize) + posoff;
    ...
    public float calculateRadiusSquared(float centerX, float centerY, float centerZ, int offset, int count, Matrix4 transform) {
        ...
        if (i == 1) {
            for (int i2 = offset; i2 < end; i2++) {
                this.tmpV.set(verts.get((index.get(i2) * vertexSize) + posoff), 0.0f, 0.0f);
                if (transform != null) {
                    this.tmpV.mul(transform);
                }
            ...
            int i4 = offset;
            while (i4 < end) {
                int idx2 = (index.get(i4) * vertexSize) + posoff;
